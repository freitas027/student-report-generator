...
fs.createReadStream('Example_ Grade Data - Sheet1.csv')
  ...
  .on('end', () => {
    ...
    results.forEach((row) => {
      ...
      const examCriteria = new ExamCriteria(row.Code, row.Exam, row['%'], row.Grade);
    ...
    for (let student of students.values()) {
      ...
      fs.writeFileSync(`${student.name.replace(/ /g, '_')}_report.html`, reportHtml);
    }
  ...
  function createCriteriaTable(studentName, criteria){
        ...
        const filteredCriteria = student.examCriterias.filter( exam => exam.criteria === criteria)
        const criteriaRows = filteredCriteria.map( examCriteria => `
            <tr>
  ...
  return `
    ...
    <body>
      <h1>${student.name}</h1>
        ...
        ${Array.from(criterias).map(criteria => createCriteriaTable(student.name, criteria)).join('')}
